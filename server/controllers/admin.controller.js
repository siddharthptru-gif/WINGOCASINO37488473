...
async function getDashboardStats(req, res) {
    try {
        ...
        const recentUsers = await query(
            `SELECT u.id, u.username, u.email, w.balance, u.created_at
             FROM users u
             JOIN wallets w ON u.id = w.user_id
...
async function getAllUsers(req, res) {
    try {
        ...
        const total = await query(
            `SELECT COUNT(*) as count FROM users u ${whereClause ? whereClause.replace('?', 'u.username LIKE ? OR u.email LIKE ?') : ''}`,
