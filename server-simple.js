...
// Balance backup system
const BALANCE_BACKUP_PATH = path.join(__dirname, 'database/balance-backup.json');

function saveBalanceBackup() {
  try {
    const users = readDB('users');
    const balanceSnapshot = {};
    users.forEach(u => {
      balanceSnapshot[u._id] = {
        balance: u.balance,
        username: u.username,
        lastUpdated: new Date().toISOString()
      };
    });
    fs.writeFileSync(BALANCE_BACKUP_PATH, JSON.stringify(balanceSnapshot, null, 2));
    console.log('Balance backup saved:', Object.keys(balanceSnapshot).length, 'users');
  } catch (error) {
    console.error('Failed to save balance backup:', error);
  }
}

function restoreBalancesFromBackup() {
  try {
    if (fs.existsSync(BALANCE_BACKUP_PATH)) {
      const backupData = JSON.parse(fs.readFileSync(BALANCE_BACKUP_PATH, 'utf8'));
      const users = readDB('users');
      let restoredCount = 0;
      
      users.forEach((user, index) => {
        if (backupData[user._id]) {
          const oldBalance = user.balance;
          users[index].balance = backupData[user._id].balance;
          console.log(`Restored ${user.username}: ₹${oldBalance} → ₹${backupData[user._id].balance}`);
          restoredCount++;
        }
      });
      ...
      if (restoredCount > 0) {
        writeDB('users', users);
        console.log('✅ Balance restoration complete:', restoredCount, 'users restored');
      }
    }
  } catch (error) {
    console.error('Failed to restore balance backup:', error);
  }
}

// Auto-backup every 30 seconds
setInterval(saveBalanceBackup, 30000);

// Restore balances on server startup
restoreBalancesFromBackup();
...
function writeDB(collection, data) {
    ...
    // Auto-save balance backup whenever users data changes
    if (collection === 'users') {
      saveBalanceBackup();
    }
}
...
app.post('/api/game/place-bet', authenticateToken, (req, res) => {
  try {
    ...
    // Calculate 1% tax
    const taxAmount = betAmount * 0.01;
    const actualBetAmount = betAmount - taxAmount;

    users[userIndex].balance -= betAmount;
    writeDB('users', users);

    // Store tax
    let taxes = readDB('taxes');
    taxes.push({
      _id: Date.now().toString(),
...
// Get user's my bets
app.get('/api/game/my-bets', authenticateToken, (req, res) => {
  try {
    const bets = readDB('bets');
    const myBets = bets.filter(b => b.userId === req.user.userId).reverse().slice(0, 20);
    res.json(myBets);
  } catch (error) {
...
app.get('/api/admin/bets', authenticateToken, (req, res) => {
  try {
    ...
    // Calculate Big/Small Analysis
    const bigBets = currentBets.filter(b => b.betType === 'big_small' && b.betOption === 'big');
    const smallBets = currentBets.filter(b => b.betType === 'big_small' && b.betOption === 'small');
    
    const bigTotalAmount = bigBets.reduce((sum, b) => sum + b.betAmount, 0);
    const smallTotalAmount = smallBets.reduce((sum, b) => sum + b.betAmount, 0);
    ...
    res.json({
      ...
      analysis: {
        big: {
          ...
          totalAmount: bigTotalAmount
        },
        small: {
          count: smallBets.length,
          totalAmount: smallTotalAmount
        }
      }
    });
  } catch (error) {
...
app.get('/api/admin/taxes', authenticateToken, (req, res) => {
  try {
    const taxes = readDB('taxes');
    const total = taxes.reduce((sum, t) => sum + t.amount, 0);
    res.json({
      taxes: taxes.reverse().slice(0, 100),
  ...
  try {
    ...
    periodBets.forEach(bet => {
      ...
      if (betIndex !== -1) {
        ...
        bets[betIndex].status = isWinning ? 'won' : 'lost';
        bets[betIndex].result = result.number;
      }
